...
&= \Pr[\mathcal{A}\left(1^n,\operatorname{Enc}_k(m_b)\right)=b] \\
&= \frac{1}{2}\Pr\nolimits_{m_0\leftarrow I_0}[\mathcal{A}\left(1^n,\operatorname{Enc}_k(m_0)\right)=0]
+ \frac{1}{2}\Pr\nolimits_{m_1\leftarrow I_1}[\mathcal{A}\left(1^n,\operatorname{Enc}_k(m_1)\right)=1]\\
&= \Pr[\mathcal{A}\left(1^n,\operatorname{Enc}_k(m)\right)=m^i]
\end{aligned}
$$
...
Every time a message m is encrypted in $\operatorname{PrivK}_{\mathcal{A},\widetilde{\Pi}}^{\mathrm{cpa}}(n)$ (either by the encryption oracle $\langle r, f(r)\oplus m \rangle$ or when the challenge ciphertext is computed $\langle r^*, f(r^*)\oplus m_b \rangle$), due to the r is chosen uniformly, there are 2 possibilities:
